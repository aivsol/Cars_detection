...
def vis_detections(im_file, class_name, dets, ax, thresh=0.5, save_img = True):
    ...
    for i in inds:
        ...
        file.write(im_file[-9:-1] + " ," + repr(bbox[0])+" ,"+ repr(bbox[1]) + " ,"+ repr(bbox[2]) +" ,"+ repr(bbox[3]) +" ,"+ str(class_id) +"\n")
...
def demo(net, image_name):
    ...
    for cls_ind, cls in enumerate(CLASSES[1:]):
        cls_ind += 1 # because we skipped backgroundplt
